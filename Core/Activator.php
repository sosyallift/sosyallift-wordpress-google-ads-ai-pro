...
class Activator {
    ...
    public static function activate(): void {
		if (!wp_next_scheduled('sl_ai_pro_cron_license_check')) {
			wp_schedule_event(time(), 'daily', 'sl_ai_pro_cron_license_check');
		}

        if (!Requirements::check()) {
            wp_die('Sosyallift AI Pro sistem gereksinimlerini karşılamıyor.');
        }
        ...
        $defaults = [
            ...
            // Uninstall settings
            'sl_ai_remove_data_on_uninstall' => 0,
            'sl_ai_uninstall_dry_run' => 0,
        ];
